...
function _RegExp(input,pattern,flag){
	...
	var str = (_get_object(input)) ? input.value : input;
	//var str=input.value;
...
var input_box = {
	...
	plus : function(Did,Doptions,Dfunction) {
		...
		if (!input_box._objs[Did]) {
			...
			input_box._objs[Did] = get_vars(Doptions,input_box._setting);			
		}
		...
		input_box._add = 1;
		input_box._num_add();
		input_box._settime = window.setInterval(input_box._num_add,200);
	},
	...
	change_options : function(Did,Doptions) {
		if (input_box._objs[Did]) {
			input_box._objs[Did] = get_vars(Doptions,input_box._objs[Did]);						
		} else {
			input_box._objs[Did] = get_vars(Doptions,input_box._setting);	
		}
	},
	set_plus_event : function(Did,Dtarget,Doptions) {
		...
		var Ctarget = _get_object(Dtarget);
	...
	_num_add : function() {
		...
		var _var;
		var box_type = jQuery("#"+input_box._target).prop("tagName").toLowerCase();

		if (box_type == "select") {
			_var = jQuery("#"+input_box._target).prop('selectedIndex');
		} else {
			_var = jQuery("#"+input_box._target).val();
		}
