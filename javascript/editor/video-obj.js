...
<div class="grid-1">
	<div class="column-1">
		<div class="padding-height-1 center">
			<div class="video-canvas-16x9" style="width:500px">
				<div>
					<iframe src="//www.youtube.com/embed/P5yHEKqx86U?rel=0" frameborder="0" allowfullscreen=""></iframe>
				</div>
			</div>
    	</div>
	</div>
...
var video_obj = {
	...
	int : function(obj) {
		...
		if (jQuery('#editWindow-video').length == 0) {
			...
			edit_window_obj.int("editWindow-video",_html,msg_msg('set_video','a'));
		...
		var mouseover_func = function(e,obj) {
			...
			if (_target) {
				...
				if (_SET["navigator"] == 'ie') { jQuery("#_video_setting_box_iframe").css({display:"block",top:m['t'],left:m['l'],width:m['w'],height:m['h']}); }
				...
				if (!jQuery(et).is('#_video_setting_box') && jQuery(et).parents('#_video_setting_box').length == 0) {					
					...
					if (_SET["navigator"] == 'ie') { jQuery("#_video_setting_box_iframe").css({display:"none"}); }					
				}
	...
	check_viedo_and_set : function() {
		...
		if (!J_video_now.is('[class*=' + this._class_front + ']') && J_video_now.parents('[class*=' + this._class_front +']').length == 0) { // video-canvas-16x9 형식의 class 명			
			...
			J_video_now.css({'width':_w,'height':_h}).removeAttr('width').removeAttr('height'); // video-canvas-auto 이므로 iframe에 width,height 값추가
		}
	},
	...
	view : function(obj) {
		...
		jQuery("#_video_setting_box").css({display:"none"});
		layer_position('editWindow-video',obj,'up',getObject('body'));				
	...
	type_change : function() {
		var _index = get_radio_index('_video_sorce_type');
		for (var i=0;i<2;i++) {
	...
	change : function() {
		...
		if (_index == 0) {
			...
			ajax_load_contents(
				_SET["video_ajax_url"] + "&video=" + encodeURIComponent(_url),
				"_video_return",
				{whenever:true},
				function() {
		...
		int : function() {
			...
			if (jQuery("#_video_setting_box").length == 0) {
				...
				'<div id="_video_setting_box" class="_setting_area_box" style="display:none;">' +
					'<div>' +
						'<a onclick="video_obj.handle.change_size();return false;" title="'+ msg_msg('fix_size','a') +'" class="edit-btn _drag-btn-3 icon-arrows-alt hd-fix no"></a>' +						
						'<a onclick="video_obj.view();return false;" title="'+ msg_msg('set_video','a') +'" class="edit-btn-2 _drag-btn-3 icon-video hd-copy no"><i class="edit-btn-popup-2"></i></a>' +
						'<a onclick="video_obj.handle.del();return false;" title="'+ msg_msg('delete','a') +'" class="edit-btn _drag-btn-3 icon-del hd-remove"></a>' +		
					'</div>' +					
				'</div>';
