	...
	aaa : {
		...
		bbb : function(obj) {
			...
			for (var i=0; i<lists.length;i++) {
				jQuery(lists[i]).attr('class','_cell');
				var num = i + 1;
				var str = '<div class="convert_unlist_style _not"><div class="valign"></div><div class="convert_unlist_icon"></div></div>\n'
					+ '<div class="content_box">\n'
					+ '<div class="convert_unlist_content">' + jQuery(lists[i]).html() + '</div>\n';
				
				if (i < (lists.length - 1)) {
					str += '<div class="convert_unlist_line2 _not"></div>\n';
				}
				str += '</div>\n';
			...
			jQuery(obj).wrap('<div class="convert_unlist_box"></div>\n');
			jQuery(obj).before('<div class="convert_unlist_line1 _not"></div>\n');
			jQuery(obj).after('<div class="convert_unlist_line3 _not"></div>\n');						
			jQuery(obj).wrap('<div class="convert_unlist"></div>\n');
			
			return jQuery(obj).parent().parent()[0];
...
var ollist_obj = {
	...
	aaa : {
		...
		bbb : function(obj) {
			...
			for (var i=0; i<lists.length;i++) {
				jQuery(lists[i]).attr('class','_cell');
				var num = i + 1;
				var str = '<div class="convert_ollist_style"><div class="convert_ollist_num"><div class="valign"></div><span class="_calculate">' + num + '</span></div></div>\n'
					+ '<div class="content_box">\n'
					+ '<div class="convert_ollist_content">' + jQuery(lists[i]).html() + '</div>\n';
				
				if (i < (lists.length - 1)) {
					str += '<div class="convert_ollist_line2 _not"></div>\n';
				}
				str += '</div>\n';
			...
			jQuery(obj).wrap('<div class="convert_ollist_box"></div>\n');
			jQuery(obj).before('<div class="convert_ollist_line1 _not"></div>\n');
			jQuery(obj).after('<div class="convert_ollist_line3 _not"></div>\n');						
			jQuery(obj).wrap('<div class="convert_ollist"></div>\n');
	...
	style_info['table_style'] = {
		'_class' : ['table-1','table-2','table-3','table-4','table-5','table-6'],		
		'_class_names' : ['T1','T2','T3','T4','T5','T6']
	};

	tab_get['table'] = ['table_style','table_head'];
...
var table_obj = {
	...
	view : function(obj) {
		...
		layer_position('editWindow-table',table_obj._now,'up',getObject('body'));							
...
'<ul class="_edit-list-1">' +
	...
	'<li>' +
		'<dl class="_edit-dl-2">' +
			'<dt>'+ msg_msg('merge','a') +'/'+ msg_msg('divide','a') +'</dt>' +
			'<dd>' +
				'<ul class="_edit-ul-1">' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-join" onclick="table_obj.join();return false" title="'+ msg_msg('table_btn_1','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-divide_vertical" onclick="table_obj.divide.vertical();return false" title="'+ msg_msg('table_btn_2','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-divide_horizontal" onclick="table_obj.divide.horizontal();return false" title="'+ msg_msg('table_btn_3','a') +'"></a></li>' +										
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-same_width" onclick="table_obj.same_width();return false" title="'+ msg_msg('table_btn_4','a') +'"></a></li>' +					
				'</ul>' +
			'</dd>' +
		'</dl>' +
	'</li>' +	
	'<li>' +
		'<dl class="_edit-dl-2">' +
			'<dt>'+ msg_msg('set_order','a') +'</dt>' +
			'<dd>' +
				'<ul class="_edit-ul-1">' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-x_sortable" onclick="table_obj.x_sortable.int();return false" title="'+ msg_msg('table_btn_5','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-y_sortable" onclick="table_obj.y_sortable.int();return false" title="'+ msg_msg('table_btn_6','a') +'"></a></li>' +			
				'</ul>' +
			'</dd>' +
		'</dl>' +
	'</li>' +	
	'<li>' +
		'<dl class="_edit-dl-2">' +
			'<dt>'+msg_msg('insert','a')+'</dt>' +
			'<dd>' +
				'<ul class="_edit-ul-1">' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-after_vertical" onclick="table_obj.insert.after_vertical();return false" title="'+ msg_msg('table_btn_7','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-before_vertical" onclick="table_obj.insert.before_vertical();return false" title="'+ msg_msg('table_btn_8','a') +'"></a></li>' +								
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-after_horizontal" onclick="table_obj.insert.after_horizontal();return false" title="'+ msg_msg('table_btn_9','a') +'"></a></li>' +					
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-before_horizontal" onclick="table_obj.insert.before_horizontal();return false" title="'+ msg_msg('table_btn_10','a') +'"></a></li>' +							
				'</ul>' +
			'</dd>' +
		'</dl>' +
	'</li>' +	
	'<li>' +
		'<dl class="_edit-dl-2">' +
			'<dt>'+ msg_msg('delete','a') +'</dt>' +
			'<dd>' +
				'<ul class="_edit-ul-1">' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-remove_horizontal" onclick="table_obj.remove.horizontal();return false" title="'+ msg_msg('table_btn_11','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-remove_vertical" onclick="table_obj.remove.vertical();return false" title="'+ msg_msg('table_btn_12','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _drag-btn-3 icon2-table-remove_table" onclick="table_obj.remove.table();return false" title="'+ msg_msg('table_btn_13','a') +'"></a></li>' +													
				'</ul>' +
			'</dd>' +
		'</dl>' +
	'</li>' +
	'<li>' +
		'<dl class="_edit-dl-2">' +
			...
			'<dd>' +
				'<ul class="_edit-ul-1">' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-remove_horizontal" onclick="table_obj.convert.thead();return false" title="&lt;thead&gt; '+ msg_msg('convert','a') +'"></a></li>' +
					'<li><a href="#" class="edit-btn-8 _table-btn _drag-btn-3 icon2-table-remove_vertical" onclick="table_obj.convert.tfoot();return false" title="&lt;tfoot&gt; '+ msg_msg('convert','a') +'"></a></li>' +										
				'</ul>' +
			'</dd>' +
		'</dl>' +
	'</li>' +	
	'<li>' +
		'<dl class="_edit-dl-1">' +
			'<dt><label for="edit_table_caption">*'+ msg_msg('caption','a') +'(caption)</label><span class="help-icon edit-btn-etc" title="'+ msg_msg('msg_23','a') +'">?</span></dt>' +
			'<dd><input type="text" id="edit_table_caption" value="" class="_drag-btn-3" /></dd>' +
		'</dl>' +																																				
	'</li>' +							
	'<li>' +
		'<dl class="_edit-dl-1">' +
			'<dt><label for="edit_table_summary">'+ msg_msg('summary','a') +'(summary)</label><span class="help-icon edit-btn-etc" title="'+ msg_msg('msg_24','a') +'">?</span></dt>' +
...
'<button onclick="table_obj.property_change()" class="curtain-btn editor-btn-yes _drag-btn-3">OK</button>';
			...
			edit_window_obj.int("editWindow-table",_html,msg_msg('table_setting','a'));

			for (var i=0;i<EHASH.colors_class.length;i++) {
				jQuery("#_editor_table_head_color").append('<option value="' + EHASH.colors_class[i] + '">' + EHASH.colors_class[i] + '</option>');
			}

			if (jQuery('#editor-table-fix').length == 0) {			
				var _html = '<div id="editor-table-fix" class="handle_width _m9editor no" style="display:none"><a href="#" onclick="table_obj.view();return false;" class="setting-btn-1">'+ msg_msg('table_setting','a') +'</a></div>';				
		...
		var mouseover_func = function(e,obj) {
			...
			if (_target) {
				...
				layer_position("editor-table-fix",_target,"up");		
		...
		jQuery(document).on('mousedown',function(e) {
			...
			if (!get_is_parents(et,'._m9editor','.m9_editor_box')) {					
				//jQuery('.selected_table').removeClass('selected_table');							
			}			
	...
	// 합치기
		...
		if (_hash['num'] < 2) {
			alert(msg_msg('msg_38','a')); // '합치기는 표의 두칸이상이여야합니다!\n드래그하여 선택해주세요!'
			return false;					
		}
		if (_hash['count'] != 1) {
			alert(msg_msg('msg_39','a')); // '합치기는 표의 \n머리글[thead], 내용글[tbody], 바닥글[tfoot]\n끼리만 가능합니다!'
			return false;					
		}

		var checking = table_obj.get_selected(obj);
		if (!checking) {
			alert(msg_msg('msg_40','a')); // '드래그 범위가 잘못되었습니다!\n[사각형]형태로 드래그해주세요!'
			return false;	
		}
	...
	_table_check : function(obj) {
		...
		// colspan 비교/수정
		var _array = table_obj._aaa_check(obj);
	...
	convert : {
		...
		thead : function() {
			...
			if (jQuery(obj).find('thead').length > 0) {
				alert(msg_msg('msg_41','a')); // '<thead>가 존재하여 변경할 수 없습니다!'
				return false;
			}
			...
			if (tr_count != 1) {
				alert(msg_msg('msg_42','a')); // '한줄만 가능합니다!'
				return false;
			}
			...
			jQuery(obj).find('thead').append(_selected.parent('tr'));	
			jQuery(obj).find('thead').find('th,td').each(function() {
				jQuery(this).prop('scope','col');
				changeTag(this,'th');
			});
		...
		tfoot : function() {
			...
			if (jQuery(obj).find('tfoot').length > 0) {
				alert(msg_msg('msg_43','a')); // '<tfoot>가 존재하여 변경할 수 없습니다!'
				return false;	
			}
			...
			jQuery(obj).find('tfoot').append(_selected.parent('tr'));	
	...
	divide : {
		...
		vertical : function() {
			...
			var _hash = table_obj._check_group(obj);
			if (_hash['num'] < 1) {
				alert(msg_msg('msg_44','a')); // '수직나누기는 한칸이상 선택되어져야합니다!'
				return false;					
			}
			...
			jQuery(obj).find('.selected_tag').each(function(){
				...
				if (colspan > 1) {
					...
					var _array = table_obj._aaa_check(obj,_selected_one);
		...
		horizontal : function() {
			...
			var _hash = table_obj._check_group(obj);
			if (_hash['num'] < 1) {
				alert(msg_msg('msg_45','a')); // '수평나누기는 한칸이상 선택되어져야합니다!'
				return false;					
			}
	...
	'x_sortable' : {
		...
		'int' : function() {
			...
			for (var x=0;x<cells.length;x++) {
				...
				var w = _get_num(jQuery(_handles[num]).css('left'));				
				var _w = w - total_w;
	...
	'y_sortable' : {
		...
		'int' : function() {
			...
			var obj = table_obj._now;
			var _top1 = getRealOffsetTop(obj);	
			...
			var _top2 = getRealOffsetTop(obj);						
			...
			for (var y=0;y<cells.length;y++) {
				...
				for (var i=0;i<cells[y];i++) {
					_h += _get_num(_handles.eq(_tr_index).css('height'));
					_tr_index++;
				}
				var color_index = y % color_length;
				_html += '<a class="_not" style="height:' + _h + 'px" data-order="' + _index + '"><span style="height:' + _h + 'px;background:' + table_obj.color_set[color_index] + '"><i class="valign"></i>' + (y+1) + '</span></a>';		
				_index++;
			}	
			_html += '</div>\n';
		...
		'get_rowspan_list' :function(_start) {
			...
			var re_cells = [];
			for (var y=0;y<cells.length;y++) {	
				re_cells.push(cells[y]);
				if (cells[y] > 1) { y += cells[y] - 1; } 
			}
	...
	insert : {
		...
		before_horizontal : function() {
			...
			var _selected = jQuery(obj).find('.selected_tag');
			var _array = table_obj._aaa_check(obj,jQuery(obj).find('.selected_tag')[0]);
			var _y = _array['y'];
	...
	// 테이블 셀 넓이 슬라이드
		...
		'int' : function(obj) {
			...
			var _total_w = _get_num(jQuery(obj).css("width"));

			var values = [];
			var _sum = 0;
			var colgroups = jQuery(obj).find('colgroup').children();
			colgroups.each(function(i) {
				var _w = _get_num(jQuery(this).attr('width'));
				values[i] = _w + _sum;
				_sum += _w;
			});
	...
	select_table : function(obj) {
	    ...
	    jQuery(obj).selectable({
			...
			stop: function() {	
				...
				table_obj.btn_setting();
				//editor._now = jQuery(obj).find('.selected_tag');
				//element_obj._now = jQuery(obj).find('.selected_tag');//[0];
	 		}
	    });
	...
	'set_colgroup' : {
		...
		'int' : function(obj,num) {
			...
			var _return = [];
			//if (jQuery(obj).find('colgroup').length == 0) { // 리사이징되면 틀어지므로 새롭게 생성
				var _html = '<colgroup>\n';	
				for (var i=0;i<_array['t_x'];i++) {
					var _w = (cols_w[i]) ? cols_w[i] : _average;
					//var _w2 = Math.round((_w * 100) / _total_w); // percent 로 처리
	...
	'get_td_width' : function(_target) {
		return _get_num(jQuery(_target).css("width"));
	}
