var grid_obj = {
	...
	_grids : ".grid-1,.grid-2,.grid-3,.grid-4,.grid-5,.grid-6",
	_columns : ".column-1,.column-2,.column-3,.column-4,.column-5,.column-6",
	_ignore : "#editMenu-grid,.editMenu-column,#alt_view0,._setting_area_box",
	_grid_before : false,	
	...
	int : function(_id) {
		...
		if (jQuery('#editMenu-grid').length == 0) {	
			...
			'<ul>' +
				//'<li><a onclick="grid_obj.mode.column.int();return false;" alt="'+ msg_msg('set_type','a') +'" title="'+ msg_msg('msg_4','a') +'" class="edit-btn icon-columns-set hd-fix"></a></li>' +			
				'<li><a onclick="grid_obj.mode.column.int();return false;" alt="'+ msg_msg('column_set','a') +'" title="'+ msg_msg('msg_72','a') +'" class="edit-btn icon-columns-set hd-fix"></a></li>' +						
				'<li><a onclick="grid_obj.spacing.change();return false;" alt="'+ msg_msg('set_type','a') +'" title="'+ msg_msg('msg_4','a') +'" class="edit-btn icon-cellspacing hd-fix"></a></li>' +				
				'<li><a onclick="grid_obj.column.add_column();return false;" alt="'+ msg_msg('add_column','a') +'" title="'+ msg_msg('msg_5','a') +'" class="edit-btn icon-plus-right hd-copy"></a></li>' +		
				'<li><a onclick="grid_obj.column.add_grid();return false;" alt="'+ msg_msg('add_grid','a') +'" title="'+ msg_msg('msg_6','a') +'" class="edit-btn icon-plus-bottom hd-copy"></a></li>' +
			'</ul>' +
			'</div>';
		...
		jQuery(document).on('mousedown',function(e) {
			...
			if (jQuery(et).parents('.m9_editor_box').length > 0 || (jQuery(et).parents(grid_obj._ignore).length > 0 || jQuery(et).is(grid_obj._ignore))) { // 편집부분에 마우스 다운이 이루어지면
				...
				obj_set_data(grid_obj._id,'edit_on',1);
			} else { // 편집부분 외에 마우스 다운이 이루어지면
				grid_obj.setting.reset();		
				obj_set_data(grid_obj._id,'edit_on',0);
			}
		});
	...
	column : {
		...
		'_hd_html' : 
			'<div class="editMenu-column handle_width _m9editor no" style="display:none">' +					
				'<div>' +
					'<ul>' +
						'<li><a onclick="return false;" alt="'+ msg_msg('set_order','a') +'" title="'+ msg_msg('msg_3','a') + ((ie_var <= 7) ? "(1초간)" : "") + '" class="edit-btn icon-arrow-width hd-mover _column-hd-mover ui-sortable-handle"></a></li>' +
						'<li><a onclick="grid_obj.column.copy(this);return false;" title="'+ msg_msg('copy','a') +'" class="edit-btn icon-plus hd-copy"></a></li>' +
						'<li><a onclick="grid_obj.column.widen();return false;" title="'+ msg_msg('widen','a') +'" class="edit-btn icon-widen hd-fix"></a></li>' +
						'<li><a onclick="grid_obj.column.narrow();return false;" title="'+ msg_msg('narrow','a') +'" class="edit-btn icon-narrow hd-fix"></a></li>' +				
						'<li><a onclick="grid_obj.html.view();return false;" title="'+ msg_msg('fix_html','a') +'" class="edit-btn icon-tag hd-html"><i class="edit-btn-popup-2"></i></a></li>' +	
						'<li><a onclick="grid_obj.column.del(this);return false;" title="'+ msg_msg('delete','a') +'" class="edit-btn icon-del hd-remove"></a></li>' +	
					'</ul>' +	
				'</div>' +
			'</div>',

		'add_grid' : function() {	
			var _grid_html = '<div class="grid-1">\n\t<div class="column-1">\n\t\t<p class="placeholder_txt">' + EHASH['placeholder_txt'] + '</p>\n\t</div>\n</div>';
		...
		'add_column' : function() {
			...
			if (sum > 6) {
				var _str = msg_msg('msg_55','a').msg_format('6'); // "총 [6]칸을 넘을 수 없습니다"
				error_msg(_str);				
				return false;	
			}		

			jQuery(grid_obj._grid_before).append('\n\t<div class="column-1 column-last grid-line-on" style="display:none">\n\t\t<p class="placeholder_txt">' + EHASH['placeholder_txt'] + '</p>\n\t</div>\n');
		...
		'copy' : function(obj) {
			...
			if (jQuery(obj).hasClass('locked')) {
				error_msg(msg_msg('msg_69','a')); // '잠금기능으로 복사 불가합니다'
				return false;	
			}
		...
		'del' : function(e) {
			...
			if (grid_num <= 1) {
				error_msg(msg_msg('msg_56','a')); // "더 이상 삭제 할 수 없습니다"
				return false;	
			}
		...
		'widen' : function() {
			...
			if (sum > 6) {
				error_msg(msg_msg('msg_57','a')); // "더이상 넓힐 수 없습니다!"
				return false;	
			}
		
			var _return = "column-" + (column_num + 1);
		...
		'narrow' : function() {
			...
			if (column_num <= 1) {
				error_msg(msg_msg('msg_58','a')); // "더이상 줄일 수 없습니다!"
				return false;	
			}
	...
	html : {
		...
		'int' : function() {
			...
			if (jQuery("#column-html").length == 0) {
				...
				'<textarea id="column_html_textarea" title="'+ msg_msg('fix_html','a') +'" alt_no="1"></textarea>' +
				'<button onclick="grid_obj.html.change()" class="curtain-btn editor-btn-yes">OK</button>' +
				'</div>';
				jQuery("body").append(_html);
			}
		...
		'view' : function() {
			...
			jQuery("#column_html_textarea").val(_html);
			m9_code_editor.set('column_html_textarea',jQuery("#column_html_textarea").val());
			curtain.show("column-html");
		...
		'convert' : function(_html) {
			...
			var J_edit_box = jQuery('#_edit_covert_box');
						
			J_edit_box[0].innerHTML = _html;
			
			J_edit_box.find(grid_obj._columns).each(function() {
				this.outerHTML = jQuery(this).find('._one-in-column').html();				
			});

			jQuery('.grid-mode-setting-box,._cols-setting-area').remove();
							
			table_obj.remove_setting(J_edit_box[0]); // 테이블처리
			//google_map_obj.remove_setting(J_edit_box[0]); // 구글지도처리

 			if (_SET["navigator"] == "ie" && ie_var < 8) {
				J_edit_box.find('.axi').each(function() {
					ie_icon_reset(this);
				});
				// ie7이하에서는 column에 width,min-width 가 생성됨 => 삭제처리
				J_edit_box.find(grid_obj._columns).css({'width':'','min-width':'','z-index':''});
 			}
			 						
			J_edit_box.find('.grid-line-on').removeClass('grid-line-on');
			J_edit_box.find('.ui-sortable').removeClass('ui-sortable');
			J_edit_box.find('.selected_tag').removeClass('selected_tag');					
			J_edit_box.find('[contenteditable]').removeAttr('contenteditable');
			J_edit_box.find('*[class=""]').removeAttr('class');
			J_edit_box.find('*[style=""]').removeAttr('style');
			mode_obj.remove_empty_style('#_edit_covert_box'); // 공백인 data-m9-X-style 삭제
			
			_html = J_edit_box.html();
			J_edit_box.remove();
		...
		'change' : function() {
			...
			jQuery("#column_html_textarea").val('\n\n' + m9_code_editor.get('column_html_textarea') + '\n\n\t');
			var J_column_before = jQuery(grid_obj._column_before);
			J_column_before.find('._one-in-column').html(jQuery("#column_html_textarea").val());	
		...
		'parser' : function(_html) { // grid 처리
			...
			jQuery('#'+_temp_id).find(grid_obj._grids).each(function(index){
				if (jQuery(this).parent('.grid-block').length > 0) {
					htmls[index] = jQuery(this).parent('.grid-block')[0].outerHTML;												
					jQuery(this).parents('.grid-block')[0].outerHTML = _separator;						
				} else {
	...
	spacing : {
		...
		_class : ['spacing-0','spacing-1','spacing-2'],
		...
		get_from_to : function(obj) { // 현재 간격Class,바꿀간격Class 두개 리턴
			...
			for (var i=0;i<this._class.length;i++) {
				...
				var _name = (_mode == '') ? this._class[i] : _mode +'-'+ this._class[i];
			...
			if (_to_num > (this._class.length - 1)) {
				_to = '';
			} else {
				var _name = (_mode == '') ? this._class[_to_num] : _mode +'-'+ this._class[_to_num];				
				_to = _name;				
			}
		...
		get : function(obj) { // 현재 간격 class 얻어오기('spacing-0','spacing-1','spacing-2')
		...
		change : function() { // 간격변경 spacing-0,1,2,0,1,2 순으로 계속 무한반복형태
	...
	set_one_grid : function(obj) { // 그리드 설정
		...
		if (J_obj.parents('.grid-block').length > 0) {

			var _block = J_obj.parents('.grid-block')[0];
	...
	reset_grid : function() {
		...
		jQuery(this._grid_before).children().each(function(index){
			...
			if (total == 1) {
				J_this.removeClass("column-first").removeClass("column-last");			
			} else {		
				if (index == 0) {
					J_this.removeClass("column-last").addClass("column-first");		
				} else if (index == (total - 1)) {
					J_this.removeClass("column-first").addClass("column-last");						
				} else {
					J_this.removeClass("column-first").removeClass("column-last");						
				}
			}	
	...
	mode : {
		...
		column : {
			...
			int : function() {
				...
				if (mode_obj.get() == '') {
					alert(msg_msg('msg_70','a'));
					return false;	
				}
	
				if (jQuery('._cols-setting-area').length > 0) {
					alert(msg_msg('msg_71','a'));
					return false;	
				}
				...
				jQuery(grid_obj._grid_before).append('<div class="grid-mode-setting-box"><button class="grid-mode-btn editor-btn-yes" onclick="grid_obj.mode.column.compelete();return false">'+msg_msg('fix','a')+'</button></div>');
				
				_cols.each(function() {
		
					//var _html = '<div class="_cols-setting-area" style="position:absolute;top:0;left:0;width:100%;height:100%;displ1ay:table;background:rgba(255, 0, 0, 0.3)"><div style="disp1lay:table-cell;vertical-align:middle;text-align:center">';
					var _html = '<div class="_cols-setting-area"><div><div>';
										
					_html += '<select onchange="grid_obj.mode.column.change(this);return false" class="_w_kind">';
