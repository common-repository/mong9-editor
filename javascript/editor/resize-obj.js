...
var resize_obj = {
	...
	int : function() {
		...
		if (jQuery("#_resize_area").length == 0) {
			...
			var _html = '<div id="_resize_area" style="display:none" title="'+ msg_msg('msg_8','a') +'">' +
			'<span id="_resize_area_msg">_</span>' +
			'<div style="position:absolute;top:0;left:0;">' +
			'<a href="#" onclick="resize_obj.optimization();return false;" title="'+ msg_msg('msg_7','a') +'" class="edit-btn icon-arrows-alt hd-fix no"></a>' +
			'</div>' + 
			'</div>';
	...
	optimization : function(){
		...
		if (J_resize_now.hasClass('m9-img-box')) { // 배경형 이미지일 경우
			J_resize_now.css({'width':'','height':''});	
		} else {
			J_resize_now.css({'width':'','height':'','max-width':'','max-height':'','min-width':'','min-height':''});	
		}
	...
	change : function(_target,_func) {
		...
		var _w = parseInt(_get_num(m['w']));
		var _h = parseInt(_get_num(m['h']));
		...
		//var aspectRatio = (_tag_name == "img" || jQuery(_target).hasClass('aspectRatio') || jQuery(_target).hasClass('m9-img-box')) ? true : false;
		var aspectRatio = (_tag_name == "img" || J_target.hasClass('_aspectRatio') || J_target.hasClass('circle')) ? true : false;		

		var _mx = get_inline_style(_target,'max-width');	
		var _img_p_w = 0; // image's possible width
		if (_mx != '') {
			_img_p_w = _get_num(_mx);
			_img_p_w = jQuery(_target).css('max-width');
		} else {
			//_img_p_w = _get_num(jQuery(_target).parent().css("width"));					
			_img_p_w = _get_num(J_target.parents(editor.editable).css("width"));				
		}
...
//				jQuery(_target).css({width:_w2,height:_h2});			

				element_obj.change_css_by_mode('width',_w2);
				element_obj.change_css_by_mode('height',_h2);

				var _l = getRealOffsetLeft(_target) + "px";
				var _t = getRealOffsetTop(_target) + "px";
			...
			stop : function(event,ui) {
				...
				if (del_width == 1) { // div등
		    		...
		    		var _w = parseInt(ui.size.width);
		    		var _h = parseInt(ui.size.height);
					J_target.css('width',_w);	
					J_target.css('max-width','100%');						
					J_target.css('height','');
					J_target.css('min-height',_h);
				...
				if (_mode != '') {
					...
					var _key = 'data-' + mode_obj.get_m9_mode_style_name(_mode);
					...
					var _css = mode_obj.get_style_by_str_array([Dvar,J_target.attr('style')]); // 스타일 합치기
